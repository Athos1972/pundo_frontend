...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const pageUrl = `${siteUrl}/${lang}/legal/terms`
  const title = `${tr.page_title_terms} — ${brand.name}`
  const description = tr.meta_desc_terms
...
export default async function TermsPage({ params }: Props) {
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-6 sm:py-12">
      <BackButton />
      <h1 className="text-2xl font-bold mb-4 sm:mb-8">{tr.page_title_terms}</h1>
