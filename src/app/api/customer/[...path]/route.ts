// ─── Customer Catch-All Proxy Route Handler ────────────────────────────────────
// Reads the HttpOnly customer_token cookie and proxies authenticated requests
// to the backend at /api/v1/customer/... and /api/v1/admin/reviews/...
// Client Components call /api/customer/... (same-origin), this handler attaches
// Authorization and forwards to BACKEND_URL.
...
async function proxy(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  const url = `${BACKEND}/api/v1/${targetPath}${search}`
