...
- Name: admin.spec.ts >> E2E-19: Admin URL Structure >> admin/shops URL loads (redirects to login when unauth)
- Location: e2e/admin.spec.ts:352:7
...
Call log:
  - navigating to "http://127.0.0.1:3500/admin/shops", waiting until "load"
...
```ts
  253 |   })
  254 | 
  255 |   test('offers list has shop_id field (input[name="shop_id"])', async ({ page }) => {
  256 |     const ok = await goAdminPageWithLogin(page, '/admin/offers')
  257 |     if (!ok) test.skip()
  258 |     const shopIdInput = page.locator('input[name="shop_id"]')
  259 |     await expect(shopIdInput).toBeVisible()
  260 |     expect(await shopIdInput.getAttribute('type')).toBe('number')
  261 |   })
  262 | })
  263 | 
  264 | // ── E2E-17: Admin Categories Page (Tree View + 3-field search) ───────────────
  265 | 
  266 | test.describe('E2E-17: Admin Categories Page', () => {
  267 |   test.skip(() => isDevServer(), 'Authenticated admin tests require test stack (port 3002)')
  268 | 
  269 |   test('categories page renders when authenticated', async ({ page }) => {
  270 |     const ok = await goAdminPageWithLogin(page, '/admin/categories')
  271 |     if (!ok) test.skip()
  272 |     expect(page.url()).toContain('/admin/categories')
  273 |   })
  274 | 
