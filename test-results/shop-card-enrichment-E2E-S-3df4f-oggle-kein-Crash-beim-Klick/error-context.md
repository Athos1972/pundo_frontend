...
- Name: shop-card-enrichment.spec.ts >> E2E-S7: Neue Filter-Chips >> Parking-Chip toggle: kein Crash beim Klick
- Location: e2e/shop-card-enrichment.spec.ts:368:7
...
Expected length: 0
Received length: 1
Received array:  ["WebSocket connection to 'ws://127.0.0.1:3500/_next/webpack-hmr?id=iIG1RRM3lUUJBAlKrmP8M' failed: Error during WebSocket handshake: net::ERR_INVALID_HTTP_RESPONSE"]
```
...
```ts
  276 | 
  277 |   test('Shop-Detail-Seite zeigt große Avatar (80×80px)', async ({ page }) => {
  278 |     await page.goto('/shops')
  279 |     await page.waitForLoadState('networkidle')
  280 |     const firstShopLink = page.locator('a[href^="/shops/"]').first()
  281 |     if (!await firstShopLink.isVisible()) return // leere DB
  282 | 
  283 |     await firstShopLink.click()
